...
import ui.cli_view # pylint: disable=C0413
from model.classes import User, WaterLog, WeightLog # pylint: disable=C0413
from model.database import Database # pylint: disable=C0413
...
def main():
    ...
    if not db.get_all_users(): # pylint: disable=no-value-for-parameter
        ...
        try:
            db.add_user(initial_user.name, initial_user.birthdate, initial_user.height_in_cm, initial_user.gender, initial_user.fitness_lvl) # pylint: disable=no-value-for-parameter, disable=line-too-long
    ...
    def update_biometrical_information():
        ...
        birthdate, height_in_cm, gender, fitness_lvl = ui.cli_view.change_user_information(class_user) # pylint: disable=E1111
        class_user.update_biometrical_data(birthdate=birthdate,
                                        height_in_cm=height_in_cm,
                                        gender=gender,
                                        fitness_lvl=fitness_lvl
                                        )
        db.update_user(class_user.id, class_user.name, birthdate, height_in_cm, gender, fitness_lvl) # pylint: disable=no-value-for-parameter, disable=line-too-long
    ...
    def add_weight_log():
        ...
        db.add_weight_log(class_user.id, weight_log_for_db.weight_in_kg, weight_log_for_db.timestamp) # pylint: disable=no-value-for-parameter, disable=line-too-long
