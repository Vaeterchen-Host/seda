...
def main(page: ft.Page):
    ...
    def get_full_user():
        ...
        water_logs = [WaterLog(l[0], l[2], l[3]) for l in db.get_all_water_logs() if l[1] == u[0]]
        weight_logs = [WeightLog(l[0], l[2], l[3]) for l in db.get_all_weight_logs() if l[1] == u[0]]
    ...
    welcome_text = ft.Text("Willkommen bei deiner Fitness-App!", size=40, weight="bold", color="blue")
    version_text = ft.Text("Version 0.1.| By Tobias Mignat & Sabine Steverding", size=10, color="grey500")

    # Profil / Dashboard Elemente
    bmi_display = ft.Text("BMI: --", size=25, weight="bold", color="blue")
    water_today_display = ft.Text("Heute getrunken: 0 ml", size=25, weight="bold", color="blue")
    ...
    water_today_text = ft.Text("Heute getrunken: 0 ml", size=18, weight="bold", color="blue")
    ...
    def load_water_logs():
        ...
        current_user.water_logs = [WaterLog(l[0], l[2], l[3]) for l in db.get_all_water_logs() if l[1] == current_user.id]
        ...
        for log in reversed(current_user.water_logs):
            water_list.controls.append(
                ft.ListTile(
                    ...
                    trailing=ft.TextButton("Löschen", on_click=lambda e, lid=log.id: delete_water(lid))
                )
            )
    ...
    def load_weight_logs():
        ...
        current_user.weight_logs = [WeightLog(l[0], l[2], l[3]) for l in db.get_all_weight_logs() if l[1] == current_user.id]
        ...
        for log in reversed(current_user.weight_logs):
            weight_list.controls.append(
                ft.ListTile(
                    ...
                    subtitle=ft.Text(f"{log.timestamp}"),
                    trailing=ft.TextButton("Löschen", on_click=lambda e, lid=log.id: delete_weight(lid))
                )
            )
        update_bmi_view()
    ...
    edit_gender = ft.Dropdown(
        label="Geschlecht",
        options=[ft.dropdown.Option("m"), ft.dropdown.Option("f"), ft.dropdown.Option("d")],
    ...
    def save_biometrical_data(e):
        try:
            ...
            current_user.update_biometrical_data(
                height_in_cm=new_height, 
                gender=new_gender, 
                fitness_lvl=new_fitness
            )

            db.update_user(
                current_user.id, current_user.name, current_user.birthdate, 
                current_user.height_in_cm, current_user.gender, current_user.fitness_lvl
            )

            # UI im Dashboard aktualisieren
            user_info_column.controls[0].value = f"Name: {current_user.name}"
            user_info_column.controls[2].value = f"Größe: {current_user.height_in_cm} cm"
    ...
    edit_dialog = ft.AlertDialog(
        ...
        content=ft.Column([edit_name, edit_height, edit_gender, edit_fitness], tight=True),
        actions=[
            ft.Button("Speichern", on_click=save_biometrical_data),
            ft.TextButton("Abbrechen", on_click=lambda _: (setattr(edit_dialog, "open", False), page.update()))
        ]
    )
    ...
    profil_view = ft.Column([
        ft.Card(content=ft.Container(padding=20, content=ft.Column([
            ...
            bmi_display,
            ft.Divider(),
            ft.Button("Biometrische Daten ändern", on_click=open_edit_dialog)
        ]))),
    ...
    confirm_dialog = ft.AlertDialog(
        ...
        actions=[
            ...
            ft.TextButton("Abbrechen", on_click=lambda _: (setattr(confirm_dialog, "open", False), page.update())),
        ]
    )
